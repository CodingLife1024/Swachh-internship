...
import threading
from csvwrite import process_csv, receive_message, send_message
import logging
import sys
...
LAST_AQI_LIST = [0]*10
LAST_HUMIDITY_LIST = [0]*10
LAST_PRESSURE_LIST = [0]*10
LAST_TEMPERATURE_LIST = [0]*10
LAST_WIND_LIST = [0]*10
LAST_RELATIVE_LIST = [0]*10
LAST_VEHICLE_LIST = [0]*10
...
def run_csv_processing():
    process_csv()  # Call your process_csv() function here
...
def get_non_empty_rows_after(csv_file_path, row_name, row_number):
    non_empty_values = []
...
# Gets non empty times after integer row_number from the csv file and adds the values of the time the rows are read to the global list ALL_TIMES and returns a list of the new values under row_name.
def get_non_empty_times_after(csv_file_path, row_name, row_number):
    non_empty_values = []
    with open(csv_file_path, 'r') as file:
        reader = csv.DictReader(file)
        ...
        for current_row_number, row in enumerate(reader, start=1):
            if current_row_number > row_number and row[row_name] != '':
                non_empty_values.append(str(row[row_name]))
    return non_empty_values
...
def update_lists():
    ...
    ALL_TIMES = get_non_empty_times_after(FILE_PATH, 'time', LAST_NON_EMPTY_ROW)
    ALL_Y_DATA_AQI = get_non_empty_rows_after(FILE_PATH, 'aqi', LAST_NON_EMPTY_ROW)
    ALL_Y_DATA_HUMIDITY = get_non_empty_rows_after(FILE_PATH, 'humidity', LAST_NON_EMPTY_ROW)
    ALL_Y_DATA_PRESSURE = get_non_empty_rows_after(FILE_PATH, 'pressure', LAST_NON_EMPTY_ROW)
    ALL_Y_DATA_TEMPERATURE = get_non_empty_rows_after(FILE_PATH, 'temperature', LAST_NON_EMPTY_ROW)
    ALL_Y_DATA_WIND = get_non_empty_rows_after(FILE_PATH, 'windVelocity', LAST_NON_EMPTY_ROW)
    ALL_Y_DATA_RELATIVE = get_non_empty_rows_after(FILE_PATH, 'relativeVelocity', LAST_NON_EMPTY_ROW)
    ALL_Y_DATA_VEHICLE = get_non_empty_rows_after(FILE_PATH, 'vehicleVelocity', LAST_NON_EMPTY_ROW)
    START_ROW = 0
    ...
    logger.info("Last non-empty row is %s", LAST_NON_EMPTY_ROW)
    logger.info("All times are %s", ALL_TIMES)
    logger.info("All AQI is %s", ALL_Y_DATA_AQI)
    logger.info("All Humidity is %s", ALL_Y_DATA_HUMIDITY)
    logger.info("All Pressure is %s", ALL_Y_DATA_PRESSURE)
    logger.info("All Relative Velocity is %s", ALL_Y_DATA_RELATIVE)
    logger.info("All Temperature is %s", ALL_Y_DATA_TEMPERATURE)
    logger.info("All Vehicle Velocity is %s", ALL_Y_DATA_VEHICLE)
    logger.info("All Wind Velocity is %s", ALL_Y_DATA_WIND)
    logger.info("Start row is %s", START_ROW)
...
def update_graph(ax, title, ylabel, row_name, lower_end, higher_end, skip_interval, i):
    ...
    global LOG_ON
    global START_ROW
    logger.info("x data is %s", x_data)
    logger.info("y data is %s", y_data)
    if LOG_ON == 1:
        print(x_data)
        print(y_data)
...
def update_statistics():
    ...
    if START_ROW >= len(ALL_Y_DATA_AQI):
        ...
        timeValue = ALL_TIMES[START_ROW]
        CURR_TIME = timeValue[:-7]
        timee0.configure(text=timeValue[:-7])
        timee.configure(text=timeValue[:-7])
...
app.geometry("{0}x{1}+0+0".format(width, height))  # Sets window size to fill the screen
app.title("Control Side GUI")
...
timeLabel0 = customtkinter.CTkLabel(master=left_section, font=("Courier", 18, "bold"), text="Current Time:") # AQI
timeLabel0.pack()

timee0 = customtkinter.CTkLabel(master=left_section, font=("Courier", 18), text="")
timee0.pack()
...
figure1 = plt.figure(figsize=(2, 2))
subplot1 = figure1.add_subplot(111)

figure2 = plt.figure(figsize=(2, 2))
subplot2 = figure2.add_subplot(111)

figure3 = plt.figure(figsize=(2, 2))
subplot3 = figure3.add_subplot(111)

figure4 = plt.figure(figsize=(2, 2))
subplot4 = figure4.add_subplot(111)

figure5 = plt.figure(figsize=(2, 2))
subplot5 = figure5.add_subplot(111)

figure6 = plt.figure(figsize=(2, 2))
subplot6 = figure6.add_subplot(111)

figure7 = plt.figure(figsize=(2, 2))
...
canvas2 = tkagg.FigureCanvasTkAgg(figure2, master=right_section)
canvas2.get_tk_widget().grid(row=0, column=1, padx=10, pady=10)

canvas3 = tkagg.FigureCanvasTkAgg(figure3, master=right_section)
canvas3.get_tk_widget().grid(row=0, column=2, padx=10, pady=10)

canvas4 = tkagg.FigureCanvasTkAgg(figure4, master=right_section)
canvas4.get_tk_widget().grid(row=1, column=0, padx=10, pady=10)

canvas5 = tkagg.FigureCanvasTkAgg(figure5, master=right_section)
canvas5.get_tk_widget().grid(row=1, column=1, padx=10, pady=10)

canvas6 = tkagg.FigureCanvasTkAgg(figure6, master=right_section)
canvas6.get_tk_widget().grid(row=1, column=2, padx=10, pady=10)

canvas7 = tkagg.FigureCanvasTkAgg(figure7, master=right_section)
canvas7.get_tk_widget().grid(row=1, column=3, padx=10, pady=10)
...
timeLabel = customtkinter.CTkLabel(master=super_right_section, font=("Courier", 18, "bold"), text="Current Time:") # AQI
timeLabel.grid(row=0, column=0, padx=10, pady=(10, 0))

timee = customtkinter.CTkLabel(master=super_right_section, font=("Courier", 18), text="")
timee.grid(row=1, column=0, padx=10, pady=(10, 0))

label_frequency = customtkinter.CTkLabel(master=super_right_section, text="Frequency:", font=("Courier", 18, "bold"))
label_frequency.grid(row=2, column=0, padx=10, pady=(10, 0))
...
frequency.grid(row=3, column=0, padx=10, pady=10)

label_theme = customtkinter.CTkLabel(master=super_right_section, text="Theme:", font=("Courier", 18, "bold"))
label_theme.grid(row=4, column=0, padx=10, pady=(10, 0))

theme_var = customtkinter.StringVar(value="Dark")
themee = customtkinter.CTkOptionMenu(master=super_right_section, values=["Dark", "Light", "System"], variable=theme_var, command=change_theme)
themee.grid(row=5, column=0, padx=10, pady=10)

# Settings
def submit_fan_speed():
    fan_speed = entryFan.get()  # Get the value from the entry box
    send_message("fanspeedchange", fan_speed)
    show_custom_messagebox("Notification", "Fan speed submitted successfully!")

def submit_speed():
    wind_speed = entryWindSpeed.get()  # Get the value from the entry box
    send_message("speedchange", wind_speed)
    show_custom_messagebox("Notification", "Wind speed submitted successfully!")

fanSpeed = customtkinter.CTkLabel(master=tabview.tab("Settings"), text="Change fan speed: ") #pressure
fanSpeed.pack()

frameFan = customtkinter.CTkFrame(master=tabview.tab("Settings"))
frameFan.pack(padx=20, pady=20, side="top")  # Place the frame at the top

entryFan = customtkinter.CTkEntry(frameFan)
entryFan.pack(side="left", padx=0)
entryFan.insert(0, "")

buttonFan = customtkinter.CTkButton(frameFan, text="Submit", command=submit_fan_speed)
buttonFan.pack(side="left", padx=0)

windSpeed = customtkinter.CTkLabel(master=tabview.tab("Settings"), text="Change wind speed setting: ") #pressure
windSpeed.pack()

frameWindSpeed = customtkinter.CTkFrame(master=tabview.tab("Settings"))
frameWindSpeed.pack(padx=20, pady=20)  # Place the frame at the bottom

entryWindSpeed = customtkinter.CTkEntry(frameWindSpeed)
entryWindSpeed.pack(side="left", padx=0)
entryWindSpeed.insert(0, "")

buttonWindSpeed = customtkinter.CTkButton(frameWindSpeed, text="Submit", command=submit_speed)
buttonWindSpeed.pack(side="left", padx=0)

stopButton = customtkinter.CTkButton(master=tabview.tab("Settings"), text="Emergency Stop", command=submit_emergency_stop)
...
def update_messages():
    ...
    for i in range(len(RECEIVED_MESSAGES)):
        ...
        label2 = customtkinter.CTkLabel(master=sub_frame2, font=("Courier", 18, "bold"), text=title)
        ...
        label3 = customtkinter.CTkLabel(master=sub_frame3, font=("Courier", 18, "bold"), text=RECEIVED_MESSAGES[i][2])
        label3.pack(fill="both", expand=True, padx=50, anchor='center')

        # Align subframes to the center
        sub_frame1.grid_columnconfigure(0, weight=0)
        sub_frame1.grid_rowconfigure(0, weight=0)
...
# Update the graphs periodically
def schedule_graph_update():
    global START_ROW
    global LOG_ON
    global CURR_TIME

    update_graph(subplot1, "AQI" + "_" + CURR_TIME, "", "aqi", 0, 500, 100, START_ROW)  # Update the first graph
    update_graph(subplot2, "Temp.(°C)" + "_" + CURR_TIME, "Celcius", "temperature", -5, 55, 10, START_ROW)  # Update the second graph
    update_graph(subplot3, "Humidity" + "_" + CURR_TIME, "liter", "humidity", 0, 100, 20, START_ROW)
    update_graph(subplot4, "Vehicle Speed(kmph):" + "_" + CURR_TIME, "liter", "vehicleVelocity", 0, 100, 20, START_ROW)
    update_graph(subplot5, "Relative Speed(kmph)" + "_" + CURR_TIME, "liter", "relativeVelocity", 0, 180, 36, START_ROW)
    update_graph(subplot6, "Wind Speed(kmph)" + "_" + CURR_TIME, "liter", "windVelocity", 0, 70, 14, START_ROW)
    update_graph(subplot7, "Pressure(hPa)" + "_" + CURR_TIME, "liter", "pressure", 300, 1100, 160, START_ROW)
    START_ROW += 1
    logger.info("Start row is %s", START_ROW)
    if LOG_ON == 1:
        print(START_ROW)
    app.after(FREQUENCY, lambda: schedule_graph_update())  # Update every 1 second (1000 milliseconds)
...
def run_receive_message():
    ...
    logger.info("Message List is %s", RECEIVED_MESSAGES)
    if LOG_ON == 1:
